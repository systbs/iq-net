# ============================================================================
#
#   MNIST Benchmark for IQ-NET Model Validation
#
#   This script benchmarks the five models from the IQ-NET paper on the
#   standard MNIST handwritten digit classification task.
#
#   - It adapts the sequence models to process image data by treating an
#     image as a sequence of pixel rows.
#   - It uses `torchvision` to load the MNIST dataset.
#   - It reports multiclass classification metrics for the 10 digit classes.
#
# ============================================================================
...
CONFIG = {
    'batch_size': 64, # A larger batch size is common for MNIST
    'learning_rate': 1e-4,
    'num_epochs': 5,
...
MODEL_CONFIGS = {
    ...
    'CNN_1D': {'embed_dim': 64, 'num_channels': [32, 64], 'kernel_sizes': [3, 3], 'dropout': 0.2},
}

# ===========================================
# 1. Model Implementations (Adapted for Image Rows)
# ===========================================
# NOTE: The core building blocks (_HolisticExtractor, etc.) and PositionalEncoding are the same as before.
# We will only show the final model classes with the adapted input layer.
...
# (Re-pasting the Zarvan blocks for completeness as they are unchanged)
...
# --- Adapted Model Classes ---

class ImageZarvanModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Replace Embedding with a Linear layer for pixel rows
...
class ImageLSTMModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.input_proj = nn.Linear(config['input_dim'], config['embed_dim'])
    ...
    def forward(self, x):
        ...
        # Use mean pooling over all rows for image classification
...
class ImageSimpleTransformerModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.input_proj = nn.Linear(config['input_dim'], config['embed_dim'])
        self.pos_encoder = PositionalEncoding(config['embed_dim'])
...
class ImageCNN1DModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.input_proj = nn.Linear(config['input_dim'], config['embed_dim'])
        layers = []
    ...
    def forward(self, x):
        # x is [B, SeqLen, Features] = [B, 28, 28]
        h = self.input_proj(x) # -> [B, 28, embed_dim]
        h = h.transpose(1, 2) # -> [B, embed_dim, 28]
...
def train_and_evaluate(model, train_loader, test_loader, config):
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        for images, labels in train_loop:
            # Reshape image to (batch_size, sequence_length, feature_size)
            images = images.squeeze(1).to(config['device'])
...
if __name__ == '__main__':
    ...
    print("--- 1. MNIST Dataset Loaded ---")
    print(f"Train samples: {len(train_dataset)}, Test samples: {len(test_dataset)}\n")
    
    results = {}
    models_to_test = {
        "Zarvan": ImageZarvanModel,
        "Transformer": ImageSimpleTransformerModel,
        "LSTM": ImageLSTMModel,
        "GRU": ImageGRUModel,
        "CNN_1D": ImageCNN1DModel,
    }
    ...
    for name, model_class in models_to_test.items():
        ...
        model_config['input_dim'] = CONFIG['image_dim']
        model_config['num_classes'] = 10 # MNIST has 10 classes
    ...
    print("\n" + "="*50)
    print("      MNIST Benchmark Final Results")
    print("="*50)
