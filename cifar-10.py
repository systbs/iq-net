# ============================================================================
#
#   CIFAR-10 Benchmark for IQ-NET Model Validation
#
#   This script benchmarks the five models from the IQ-NET paper on the
#   standard CIFAR-10 image classification task.
#
#   - It adapts the sequence models to process 3-channel color images by
#     treating an image as a sequence of rows, where each row's features
#     are a concatenation of the R, G, B channels.
#   - It uses `torchvision` to load the CIFAR-10 dataset.
#
# ============================================================================
...
CONFIG = {
    'batch_size': 64,
    'learning_rate': 1e-4,
    'num_epochs': 10, # CIFAR-10 is more complex, so more epochs are needed
    'image_height': 32,
    'image_width': 32,
    'num_channels': 3,
...
# ===========================================
# The core model building blocks are identical to the MNIST script.
# The final model classes are also the same, as they are parameterized by the config.
# For brevity, we will re-paste one of them and note the others are the same structure.
...
class _HolisticExtractor(nn.Module): # Unchanged
...
class _AssociativeExtractor(nn.Module): # Unchanged
...
class _SequentialExtractor(nn.Module): # Unchanged
    def __init__(self, embed_dim):
        super().__init__()
...
class _ZarvanBlock(nn.Module): # Unchanged
...
def train_and_evaluate(model, train_loader, test_loader, config):
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        for images, labels in train_loop:
            # Reshape image from (B, C, H, W) to (B, H, C*W) for sequence models
            b, c, h, w = images.shape
            images = images.permute(0, 2, 1, 3).reshape(b, h, c * w).to(config['device'])
...
if __name__ == '__main__':
    ...
    print("--- 1. CIFAR-10 Dataset Loaded ---")
    ...
    # Calculate input dimension for the sequence models
    input_dim = CONFIG['num_channels'] * CONFIG['image_width']
    ...
    for name, model_class in models_to_test.items():
        ...
        # For CIFAR-10, we only care about Accuracy and F1 for the final report
    ...
    print("\n" + "="*50)
    print("      CIFAR-10 Benchmark Final Results")
    print("="*50)
